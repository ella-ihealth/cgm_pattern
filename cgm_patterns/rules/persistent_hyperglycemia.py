"""Detect persistent hyperglycemia across recent days."""
...
@register_rule
class PersistentHyperglycemiaRule(PatternRule):
    id = "persistent_hyperglycemia"
    pattern_id = 1
    description = "TAR ≥25% (>180 mg/dL) on ≥40% of days within a 14-day window"
    version = "1.1.0"
    metadata = {
        **PATTERN_METADATA[1],
    ...
    def detect(self, window: PatternInputBundle, context: PatternContext) -> PatternDetection:
        ...
        tar_threshold = float(self.resolved_threshold(context, "tar_threshold", 0.25))
