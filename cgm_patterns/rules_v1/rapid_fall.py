"""Detect rapid glucose falls (<-60 mg/dL within 15 minutes)."""
...
@register_rule
class RapidFallRule(PatternRule):
    id = "rapid_fall"
    pattern_id = 30
    description = "≥3 days with >60 mg/dL drop within 15 minutes"
    version = "1.0.0"
    metadata = PATTERN_METADATA[30]
    ...
    def detect(self, window: PatternInputBundle, context: PatternContext) -> PatternDetection:
        ...
        fall_threshold = float(self.resolved_threshold(context, "rapid_fall_threshold", -60.0))
        days_required = int(self.resolved_threshold(context, "rapid_fall_days_required", 3))
        ...
        for day in eligible_days:
            ...
            if deltas.le(fall_threshold).any():
                idx = deltas[deltas.le(fall_threshold)].index[0]
                qualifying.append(
                    {
                        "service_date": day.service_date.isoformat(),
                        "timestamp": idx.isoformat(),
                        "drop_mg_dl": float(deltas.loc[idx]),
                    }
                )
        ...
        metrics = {
            ...
            "rapid_fall_days": len(qualifying),
            "fall_threshold": fall_threshold,
        }
        evidence = {
            "fall_examples": qualifying[:days_required],
